...
The backend serves on `http://0.0.0.0:8000` by default. Override with `BERTH_HOST` /
...
This is the full-fat profile: torch is available, all models load, no memory caps. For the
constrained profiles see [`../docker/`](../docker/) and [`../edge/`](../edge/).
