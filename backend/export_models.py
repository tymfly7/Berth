...
if ok:
    _paths = {name: path for name, path, _ in MODELS}
    for name in ok:
        if "yolo" in name:
            # YOLO NCNN export dir name derives from the weights file stem.
            print(f"  backend/models/{Path(_paths[name]).stem}_ncnn_model/  (directory)")
        else:
