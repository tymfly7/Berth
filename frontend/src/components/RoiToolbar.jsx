...
const proposalBtnStyle = (disabled) => ({
  ...
  color: disabled ? 'var(--text-muted)' : 'var(--color-proposal)',
...
export default function RoiToolbar({
  ...
  return (
    ...
    {orientEnabled && (
      <div style={{ display: 'flex', gap: 6, marginBottom: overlay ? 0 : 8, ...(overlay ? { padding: '6px 8px', background: 'var(--bg-card)', backdropFilter: 'blur(4px)', pointerEvents: 'auto' } : {}) }}>
    ...
    {/* ── ROI drawing toolbar ── */}
    <div style={{ display: 'flex', gap: 6, marginBottom: overlay ? 0 : 8, flexWrap: 'wrap', ...(overlay ? { padding: '6px 8px', background: 'var(--bg-card)', backdropFilter: 'blur(4px)', pointerEvents: 'auto' } : {}) }}>
      ...
      <input
        ...
        style={{
          ...
          border: '1px solid var(--border-strong)',
          background: 'var(--surface-2)', color: 'var(--text-muted)', outline: 'none',
        }}
    ...
    {selectedRoi && (
      <div style={{
        ...
        padding: '5px 8px',
        background: overlay ? 'var(--bg-card)' : 'var(--surface-1)',
        borderRadius: overlay ? 0 : 4,
        border: overlay ? 'none' : '1px solid var(--border-color)',
        ...
        />
        <span style={{ fontSize: '0.73rem', color: 'var(--text-muted)', margin: '0 2px' }}>type:</span>
        ...
        {selectedSpotType === 'reserved' && (
          <button
            style={{ ...typeBtnStyle(false, '#e6a817'), borderColor: '#e6a81766', color: '#e6a817' }}
            onClick={setOwner}
    ...
    {hasProposals && (
      <div style={{
        ...
        border: overlay ? 'none' : '1px solid var(--color-proposal-border)',
        background: overlay ? 'var(--bg-card)' : 'var(--color-proposal-tint)',
        backdropFilter: overlay ? 'blur(4px)' : undefined,
        borderBottom: overlay ? '1px solid var(--color-proposal-border)' : undefined,
        pointerEvents: overlay ? 'auto' : undefined,
      }}>
        <span style={{ fontSize: '0.75rem', color: 'var(--color-proposal)', marginRight: 2 }}>
        ...
        <button
          style={{ ...proposalBtnStyle(!selProp), borderColor: 'var(--border-strong)', color: 'var(--text-muted)', opacity: selProp ? 1 : 0.5 }}
          disabled={!selProp}
        ...
        <button
          style={{ ...proposalBtnStyle(false), borderColor: 'var(--border-strong)', color: 'var(--text-muted)' }}
    ...
    {/* ── Edit mode hint ── */}
    {mode === 'edit' && (
      <div style={{ fontSize: '0.72rem', color: 'var(--color-proposal)', marginBottom: 8, paddingLeft: 2 }}>
    ...
    {orientEnabled && layer === 'orientation' && (<>
      <div style={{ display: 'flex', gap: 6, marginBottom: overlay ? 0 : 8, flexWrap: 'wrap', alignItems: 'center', ...(overlay ? { padding: '6px 8px', background: 'var(--bg-card)', backdropFilter: 'blur(4px)', pointerEvents: 'auto' } : {}) }}>
